...
export default function Carousel(props: Props) {
    ...
    const forceRenderIfNeeded = useCallback(() => {
        if (!atEnd) {
            if (
                (ref.current?.scrollLeft ?? 0) <= 0 ||
                (ref.current?.scrollLeft ?? 0) + (ref.current?.clientWidth ?? 0) >=
    ...
    const handleRightClick = useCallback(() => {
        ...
        if (props.pixelsToScroll) {
            target.scroll({
                left: target.scrollLeft + props.pixelsToScroll,
                top: 0,
                behavior: "smooth",
            });
        } else {
            target.scroll({
                left: target.scrollLeft + target.clientWidth * portionOfClientWidthToScroll,
                top: 0,
                behavior: "smooth",
            });
        }
    ...
    return (
        <div
            className="relative w-full px-2 h-full"
            style={{ userSelect: "none" }}
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}>
            <div
                className="absolute top-0 bottom-0 w-10 flex items-center pointer-events-none z-20 transition-all justify-start -left-3 h-full"
                style={{
                    opacity: showLeftGradient ? "1" : "0",
                    background: "linear-gradient(90deg, #202125 0%, transparent 100%)",
                }}>
                <button
                    type="button"
            ...
            <div
                className="w-full overflow-x-scroll hide-scrollbar h-full"
                ref={ref}
                onMouseDown={handleMouseDown}
                onScroll={forceRenderIfNeeded}
                style={{ cursor: isOverflowing ? (isDragging ? "grabbing" : "grab") : "default" }}>
                <div style={{ gap: props.gap || 0 }} className="w-fit flex justify-start h-full">
                    {props.children}
                </div>
            </div>
            <div
                className="absolute top-0 bottom-0 w-10 flex items-center pointer-events-none z-20 transition-all justify-end -right-3 h-full"
                style={{
                    opacity: showRightGradient ? "1" : "0",
                    background: "linear-gradient(270deg, #202125 0%, transparent 100%)",
                }}>
                <button
                    type="button"
